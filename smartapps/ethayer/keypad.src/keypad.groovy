...
Sep 20, 2017 when setDefaultAlarm is false, insure there is a user routine code for all options
		 ...
		 unable to adjust the page install flag to true or false, all dynamic attempts end with an error 
Sep 19, 2017 Add setDefaultAlarm flag Default: True, user may set false when all four routines are set
	      reducing system overhead. Deprecates Aug 26, 2017 change commenting out sendSHMevent	
...
def mainPage() {
  dynamicPage(name: 'mainPage',title: 'Keypad Settings (optional)', install: true, uninstall: true) {
    ...
    actions?.sort()
    section('Routines') {
        def emsg="ERROR: When Keypad adjusts the ST/SHM Alarm State setting is false, all Routines must be set" 
    	def stdmsg="settings here are for this keypad only. Global keypad settings, use parent app."
	if (settings.setDefaultAlarm ==null)
      		{paragraph stdmsg}
	else
	if (setDefaultAlarm == false)
		{
		if ((armRoutine || parent.armRoutine) &&
		    (disarmRoutine || parent.disarmRoutine) &&
		    (stayRoutine || parent.stayRoutine) &&
		    (nightRoutine || parent.nightRoutine))
			{paragraph stdmsg}
		else
			{
			paragraph emsg
		...
		{paragraph stdmsg}
      input(name: 'runDefaultAlarm', title: 'When ST/SHM Alarm State changes, adjust the keypad State?', type: 'bool', defaultValue: true)
      input(name: 'armRoutine', title: 'Arm/Away routine', type: 'enum', options: actions, required: false, multiple: true, submitOnChange: true)
      input(name: 'disarmRoutine', title: 'Disarm routine', type: 'enum', options: actions, required: false, multiple: true, submitOnChange: true)
      input(name: 'stayRoutine', title: 'Arm/Stay routine', type: 'enum', options: actions, required: false, multiple: true, submitOnChange: true)
      input(name: 'nightRoutine', title: 'Arm/Night routine', type: 'enum', options: actions, required: false, multiple: true, submitOnChange: true)
      input(name: 'armDelay', title: 'Arm Delay (in seconds)', type: 'number', required: false)
//	redisplay error msg in middle of page make it more viewable
	...
	if (setDefaultAlarm == false)
      ...
      input(name: 'notifyIncorrectPin', title: 'Notify with keypad tone when incorrect code is used?', type: 'bool', required: false)
      ...
      input(name: 'setDefaultAlarm', title: 'When a valid pin changes the keypad Alarm State, adjust the ST/SHM Alarm State?\nSet to off/false when all routines are set here or in global Keypad Routines (optional), and each routine sets an Alarm State',
      			type: 'bool', defaultValue: true, required: true, submitOnChange: true)
     }
...
def execRoutine(aMap) {
  ...
  def armMode_event=armMode
  if (armMode=='night')
  	{armMode_event='stay'}
  if (settings.setDefaultAlarm==null || setDefaultAlarm)
  	{
	def event = [
